...
#   param_sets <- t(unique(t(sapply(results, function(x) unlist(x$tvs))))) |>
#     as.data.frame()
#
...
summarize_simulation_results <- function(results) {
  ...
  for (param_set in seq_along(param_sets)) {
    ...
    for (model in model_groups) {
      ...
      model_results <- param_results[grep(model, names(param_results))]

      # Calculate summary statistics for each scenario within the current model
