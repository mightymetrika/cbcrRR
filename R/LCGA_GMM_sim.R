...
#' @param covar_noise A vector to add Gaussian noise to the covar.  Default is
#' c(0,0).
...
LCGA_GMM_sim <- function(n_pers, n_time, beta_int, beta_slo_time, beta_slo_covar,
                         ...
                         mean_r, var_r, covar_noise = c(0, 0),
  ...
  dat$time <- dat$time-1
  dat$covar <- stats::rnorm(nrow(dat), covar_noise[[1]], covar_noise[[2]])
