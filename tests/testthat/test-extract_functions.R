test_that("extract_lcga_info works", {
  ...
  lcga_res <- extract_lcga_info(run_lcga_models(sim_data = sim_data))
  expect_equal(length(lcga_res), 2)
})

test_that("extract_lcga_info try catch functionality", {
  set.seed(123)
  sim_data <- LCGA_GMM_sim(n_pers = 100, n_time = 5, beta_int = 0, beta_slo_time = .1,
                           mean_i = 0, var_i = .1, mean_s = 0,
                           var_s = 0, cov_is = 0, mean_r = 0, var_r = .1, mod_name = "test_model")
...
test_that("extract_lme4_info works", {
  ...
  expect_equal(length(lme4_res), 2)
})
#
# test_that("extract_lme4_info try catch functionality", {
#   set.seed(123)
#   sim_data <- LCGA_GMM_sim(n_pers = 100, n_time = 5, beta_int = 0, beta_slo_time = .1,
#                            mean_i = 0, var_i = .1, mean_s = 0,
#                            var_s = 0, cov_is = 0, mean_r = 0, var_r = .1, mod_name = "test_model")
...
test_that("extract_sand_info works", {
  ...
  expect_equal(length(sand_res), 1)
})
...
test_that("extract_cbcRR_info works", {
  ...
  expect_equal(length(cbcRR_res), 1)
})
